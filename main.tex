...
This algorithm was validated on a comprehensive clinical dataset comprising \textbf{324 continuous markerless kinematic sequences} derived from \textbf{37 individuals} with mild-to-moderate Parkinson's disease. The experimental protocol involved 3 distinct assessment moments (baseline, immediately post-intervention, and a 1-week follow-up), with up to 3 directional turning trials per moment (the first trial requiring a right turn, the second a left turn, and the third allowing a free choice by the participant). Minor data attrition (9 out of 333 theoretical trials, yielding a 97.3\% retention rate) occurred strictly due to occasional camera occlusions or tracking artifacts. This high processing success rate underscores the robustness of the \textit{tugturn} automated pipeline when deployed in real-world clinical settings.
