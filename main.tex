...
\begin{enumerate}
  ...
  \item \textbf{Forward gait start:} first frame where a foot crosses the chair threshold (\texttt{y\_chair}).
  ...
  \item \textbf{Backward gait end:} foot crossing back below \texttt{y\_chair}; remaining frames define \texttt{sit}.
\end{enumerate}
...
A walking mask (standing posture + pelvis macro-velocity threshold) is used to suppress false events during sit-to-stand, pause, turning, and stand-to-sit. After phase segmentation, events outside \texttt{gait\_forward} and \texttt{gait\_back} are removed.
...
\begin{enumerate}
  ...
  \item Segment the trial into distinct spatial-temporal phases (\texttt{stand}, \texttt{gait\_forward}, \texttt{turn180}, \texttt{gait\_back}, \texttt{sit}). Segmentation relies on anteroposterior Y-axis thresholds and spatial tolerances to robustly identify postural transitions and the turning zone boundaries.
